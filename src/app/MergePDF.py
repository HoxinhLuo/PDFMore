...
def merge_pdf(queue: Queue, pdf_list: List[Path], merged_pdf_file: Path):
    with Document() as merged_pdf:
        for pdf_no, pdf_file in enumerate(pdf_list, start=1):
            with Document(str(pdf_file)) as pdf:
